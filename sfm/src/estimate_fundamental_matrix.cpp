...
sfm::RansacFundamentalMatrix::RansacFundamentalMatrix()
{
...
bool sfm::RansacFundamentalMatrix::determineFundamentalMatrix(const std::vector<Eigen::Vector2d> &source_points, const std::vector<Eigen::Vector2d> &target_points, int num_points, Eigen::Matrix3d &transform_matrix, bool bUseSVD)
{
	unsigned int num_points_a = source_points.size();
	unsigned int num_points_b = target_points.size();

	if (num_points_b != num_points_a)
	{
		std::cout <<"Number of points matched are not equal!"<<std::endl;
		return false;
	}

	//Preallocate space for Matrix A
	Eigen::MatrixXf mat_A;
	mat_A.resize(num_points_a, 9);

	//populate matrix with values
	for ( unsigned int i = 0; i < num_points_a; i++)
	{
...
//sfm::RansacHomography::RansacHomography(const std::vector<Eigen::Vector2d> &point_set_a, const std::vector<Eigen::Vector2d> &point_set_b)
