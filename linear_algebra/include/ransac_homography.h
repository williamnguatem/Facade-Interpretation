#ifndef _RANSAC_HOMOGRAPHY_H_
#define _RANSAC_HOMOGRAPHY_H_
...
namespace fi
{
	...
	class RansacHomography //for now I'll just wrap my code around the binaries of uni koblenz or Helmuts code based on Halkon 
	{
	...
	public:
		...
		*/
		RansacHomography(const std::vector<Eigen::Vector2d> &point_set_a , const std::vector<Eigen::Vector2d> &point_set_b);


		//Destructor
		virtual ~RansacHomography();
		...
		*/
		bool computeRansacHomography(Eigen::Matrix3d &best_homography_mat, unsigned int num_iterations, float ransac_threshold, std::vector<Eigen::Vector2d> &match_point_set_a, std::vector<Eigen::Vector2d> &match_point_set_b);
		...
		* \param sInPolyData the name of the file containing the vtkpolydata, sOutCVecData the output pcl data
		*/
		void applyHomography(const Eigen::Matrix3d &a_mat, const Eigen::Vector2d &p, Eigen::Vector2d &result);
		...
		*/
		...
		bool determineHomography(const std::vector<Eigen::Vector2d> &source_points, const std::vector<Eigen::Vector2d> &target_points, int num_points, Eigen::Matrix3d &transform_matrix, bool bUseSVD);
